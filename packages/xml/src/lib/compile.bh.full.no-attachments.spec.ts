...
describe('BrightHouse TC103 Full no-attachments', () => {
  it.skip('compiles', async () => {
    ...
    const compiled = await compile(
      input,
      options,
      'fixtures/fixture-1.tc103.full.BrightHouse.xml.ejs'
    );

    // expect(removeWhitespace(compiled[0])).toEqual(
    //   removeWhitespace(
    //     fs.readFileSync(
    //       './packages/xml/fixtures/fixture-1.tc103.full.BrightHouse.xml',
    //       'utf-8'
    //     )
    //   )
    // );
